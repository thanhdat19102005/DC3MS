...
import { RegisterReliefComponent } from '../../../components/relief/register-relief.component'; 
...
@Component({
  ...
  styleUrl: './detailed-request.component.css'
})
export class DetailedRequestComponent implements OnInit { 
  ...
  isRegisterClosing = signal(false); 
...
requestCreated = new EventEmitter<void>();
  ...
  // 🔥 KHAI BÁO FORM: Giữ các trường cũ và bổ sung đầy đủ 6 trường dữ liệu mới
  public rescueForm = new FormGroup({
    senderName: new FormControl(''), 
    phoneNumber: new FormControl(''),
    rescueType: new FormControl('Di Tản', [Validators.required]), // Mặc định theo hình ảnh của bạn
  ...
  sendRequest() {
    ...
    // Sử dụng chung Service cũ vì cấu trúc gửi FormData truyền files giữ nguyên
